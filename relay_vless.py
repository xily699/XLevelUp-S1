...
from speed_limit import throttle_upload, throttle_download
from adaptive_flow import AdaptiveQuotaGate, AdaptiveFlowControl

# ══════════════════════════════════════════════════════════════════════════════
# VLESS Relay — بهینه‌شده برای حداکثر throughput
#
# X5G.1: قبلاً کوتا به‌ازای هر چانک با await check_and_use() چک می‌شد و
# backpressure روی یک آستانه‌ی ثابت (RELAY_BUF=256KB) بود. حالا از همون
# موتور adaptive که در xhttp_siz10.py برای stream-up اثبات شده استفاده
# می‌کنیم: batch چک کوتا بر اساس نرخ واقعی سشن (EWMA) و high-water
# تطبیقی برای drain — نتیجه: await کمتر روی سشن‌های پرسرعت، و قطع سریع‌تر
# روی سشن‌هایی که کوتاشون تازه تموم شده.
# ══════════════════════════════════════════════════════════════════════════════

RELAY_BUF = 256 * 1024   # 256 KB — فقط برای تصمیم drain() سمت writer.transport استفاده می‌شه
...
async def relay_ws_to_tcp(ws: WebSocket, writer: asyncio.StreamWriter, conn_id: str, uid: str,
                           gate: AdaptiveQuotaGate, flow: AdaptiveFlowControl):
    try:
        while True:
            ...
            if not await gate.add(len(data)):
                ...
                break
            await throttle_upload(uid, len(data))
    ...
    try:
        while True:
            ...
            if not await gate.add(len(data)):
                await ws.close(code=1008, reason="quota/disabled/unknown")
                break
            await throttle_download(uid, len(data))
            connections[conn_id]["bytes"] += len(data)
...
async def websocket_tunnel(ws: WebSocket, uuid: str):
    ...
    try:
        ...
        # X5G.1: هر سشن یک AdaptiveQuotaGate جدا (چون رفتار ترافیک up/down فرق
        # داره) و یک AdaptiveFlowControl مشترک برای جهت downlink→tcp (همون
        # جایی که بیشترین حجم رد می‌شه).
        gate_up = AdaptiveQuotaGate(uuid, check_and_use)
        gate_down = AdaptiveQuotaGate(uuid, check_and_use)
        flow = AdaptiveFlowControl()

        done, pending = await asyncio.wait(
            {
                asyncio.create_task(relay_ws_to_tcp(ws, writer, conn_id, uuid, gate_up, flow)),
                asyncio.create_task(relay_tcp_to_ws(ws, reader, conn_id, uuid, gate_down)),
            },
