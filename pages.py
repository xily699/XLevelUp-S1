...
# لوگوی X5G (به‌صورت base64 داخلی، بدون نیاز به هاست خارجی)
...
<div class="wrap">
  <div class="card">
    <div class="brand">
      ...
      <div><div class="brand-name">X5G</div><div class="brand-sub">v9.8</div></div>
    </div>
    <h1>ورود به پنل</h1>
...
.nav-it{display:flex;align-items:center;gap:9px;padding:9px 14px;color:var(--t3);font-size:12.5px;cursor:pointer;border-right:2px solid transparent;transition:all .2s cubic-bezier(.3,.8,.4,1);margin:1px 6px;border-radius:8px;text-decoration:none}
.nav-it i{font-size:16px;width:18px;text-align:center;flex-shrink:0;transition:transform .2s}
.nav-it:hover{background:var(--accent-d);color:var(--t2);transform:translateX(-3px)}
.nav-it:hover i{transform:scale(1.15)}
.nav-it.on{background:var(--accent-d);color:var(--t1);border-right-color:var(--accent);font-weight:600}
.nav-it .nav-ext-badge{margin-right:auto;font-size:8px;font-weight:800;padding:2px 6px;border-radius:6px;background:rgba(34,197,94,.15);color:#22c55e}
...
.proto-card{border:1.5px solid var(--card-b);border-radius:13px;padding:13px 12px;cursor:pointer;transition:.25s cubic-bezier(.2,.8,.2,1);text-align:center;position:relative;overflow:hidden;
  background:rgba(255,255,255,.04);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px)}
[data-theme="light"] .proto-card{background:rgba(255,255,255,.55)}
.proto-card::before{content:'';position:absolute;inset:0;border-radius:13px;padding:1.5px;
  background:linear-gradient(135deg,transparent,transparent);opacity:0;transition:.25s}
.proto-card:hover{border-color:var(--card-bh);transform:translateY(-3px) scale(1.015);box-shadow:0 8px 22px rgba(0,0,0,.18)}
.proto-card.active{border-color:transparent;background:linear-gradient(160deg,var(--accent-d),rgba(255,255,255,.03));
  box-shadow:0 0 0 1px var(--accent),0 0 22px rgba(59,130,246,.35);animation:protoGlow 2.4s ease-in-out infinite}
@keyframes protoGlow{0%,100%{box-shadow:0 0 0 1px var(--accent),0 0 14px rgba(59,130,246,.25)}50%{box-shadow:0 0 0 1px var(--accent),0 0 26px rgba(59,130,246,.5)}}
...
.proto-card-icon{width:32px;height:32px;border-radius:9px;background:var(--accent-d);color:var(--accent);display:flex;align-items:center;justify-content:center;font-size:16px;margin:0 auto 8px;transition:.25s}
.proto-card.active .proto-card-icon{background:var(--accent);color:#fff;transform:scale(1.08) rotate(-4deg)}
...
.proto-card-badge{position:absolute;top:6px;right:6px;font-size:8px;font-weight:800;padding:2px 6px;border-radius:6px;background:rgba(245,158,11,.18);color:#f59e0b;letter-spacing:.3px}
.proto-card.disabled{opacity:.55;cursor:not-allowed}
.proto-card.disabled:hover{transform:none;box-shadow:none}
...
<div class="mob-top">
  <div class="ml">
    <div class="mob-logo"><img src="data:image/png;base64,__LOGO_B64__" alt="X5G"></div>
    <span class="mob-title">X5G</span>
  </div>
  <div class="mob-right">
...
<aside class="sidebar" id="sb">
  ...
  <div class="logo">
    <div class="logo-img"><img src="data:image/png;base64,__LOGO_B64__" alt="X5G"></div>
    <div><div class="logo-name">X5G</div><div class="logo-sub">v9.8</div></div>
  </div>
  <div class="nav-wrap">
    ...
    <div class="nav-it" onclick="openWorkerDashboard()"><i class="ti ti-cloud-bolt"></i> داشبورد Edge Worker <span class="nav-ext-badge">مستقل</span></div>
...
<section class="pg on" id="pg-overview">
  ...
  <div class="dash-footer">
    <span class="df-text">X5G v9.8 · Railway</span>
    ...
    <div class="cp-body">
      ...
      <div class="cp-block mb16">
        ...
        <select id="nl-proto" style="display:none">
          ...
          <option value="grpc">gRPC · multi-mode</option>
        </select>
        <div class="proto-cards">
          ...
          <div class="proto-card disabled" data-val="grpc" title="این پروتکل هنوز روی بک‌اند پیاده‌سازی نشده">
            <div class="proto-card-badge">به‌زودی</div>
            <div class="proto-card-icon"><i class="ti ti-transform"></i></div>
            <div class="proto-card-title">gRPC · multi-mode</div>
            <div class="proto-card-desc">نیاز به موتور framing اختصاصی روی Railway</div>
          </div>
        </div>
      </div>
      <div class="cp-row">
...
<section class="pg" id="pg-support">
  ...
  <div class="srv-panel">
    <div class="srv-hero">
      ...
      <div class="srv-hero-text">
        <div class="srv-hero-domain">پشتیبانی X5G</div>
        <div class="srv-hero-sub"><span class="dot dg pulse"></span> راه‌های ارتباطی با تیم X5G</div>
      </div>
    </div>
    <div class="srv-tiles">
...
function openWorkerDashboard(){
  let d=localStorage.getItem('x5g_edge_domain');
  if(!d){ d=prompt('دامنه‌ی Worker کلودفلرت رو وارد کن (بدون https://)، مثلاً: your-worker.workers.dev'); if(!d)return; localStorage.setItem('x5g_edge_domain', d.trim()); }
  window.open('https://'+d.replace(/^https?:\/\//,'').replace(/\/+$/,'')+'/dashboard', '_blank');
  closeSb();
}
...
<div class="wrap">
  <div class="top">
    <div class="brand">
      <div class="brand-img"><img src="data:image/png;base64,{LOGO_B64}" alt="X5G"></div>
      <div><div class="brand-name">X5G</div><div class="brand-sub">v9.8</div></div>
    </div>
  ...
  <div class="footer">پشتیبانی: <a href="https://t.me/X4GHUB" target="_blank">@X4GHUB</a> · X5G v9.8</div>
</div>
<script>
