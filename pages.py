...
<div class="wrap">
  <div class="card">
    <div class="brand">
      <div class="brand-img"><img src="data:image/png;base64,__LOGO_B64__" alt="X5G"></div>
      <div><div class="brand-name">X5G</div><div class="brand-sub">5.2 LevelUp</div></div>
    </div>
    <h1>ورود به پنل</h1>
...
*{margin:0;padding:0;box-sizing:border-box}
:root{
  --bg:#08090f;--bg2:#0c0e18;--bg3:#11141f;
  --card:rgba(255,255,255,0.045);--card-b:rgba(255,255,255,0.09);--card-bh:rgba(124,108,246,0.35);
  --accent:#7C6CF6;--accent2:#22D3C8;--accent-d:rgba(124,108,246,0.14);
  --green:#22C55E;--green-bg:rgba(34,197,94,0.1);--green-t:#4ADE80;
  --red:#F5455C;--red-bg:rgba(245,69,92,0.1);--red-t:#FB7185;
  --amber:#F5A524;--amber-bg:rgba(245,165,36,0.1);--amber-t:#FBBF24;
  --purple:#A78BFA;--purple-bg:rgba(167,139,250,0.1);
  --t1:#F1F0FF;--t2:#9A96C4;--t3:#5B577F;
  --sidebar-w:248px;--radius:18px;
  --glass-blur:22px;
  --shadow:0 8px 32px rgba(0,0,0,0.45);
}
[data-theme="light"]{
  --bg:#EEF0FA;--bg2:#E4E7F6;--bg3:#D8DCF2;
  --card:rgba(255,255,255,0.6);--card-b:rgba(124,108,246,0.16);--card-bh:rgba(124,108,246,0.4);
  --accent:#6D5BD0;--accent2:#0EA5A0;--accent-d:rgba(109,91,208,0.08);
...
body:before{content:'';position:fixed;inset:0;z-index:0;pointer-events:none;
  background:
    radial-gradient(circle at 8% 8%,rgba(124,108,246,.22) 0%,transparent 42%),
    radial-gradient(circle at 92% 18%,rgba(34,211,200,.16) 0%,transparent 40%),
    radial-gradient(circle at 25% 90%,rgba(245,69,140,.10) 0%,transparent 38%),
    radial-gradient(circle at 80% 85%,rgba(124,108,246,.14) 0%,transparent 40%);
}
[data-theme="light"] body:before{
  background:
    radial-gradient(circle at 8% 8%,rgba(124,108,246,.14) 0%,transparent 42%),
    radial-gradient(circle at 92% 18%,rgba(14,165,160,.10) 0%,transparent 40%),
    radial-gradient(circle at 25% 90%,rgba(245,69,140,.06) 0%,transparent 38%),
    radial-gradient(circle at 80% 85%,rgba(124,108,246,.08) 0%,transparent 40%);
}
.sidebar,.main,.topbar,.overlay{position:relative;z-index:1}
/* ── شیشه‌ای (Glassmorphism) روی همه‌ی سطوح کارتی ── */
.sidebar,.card,.cfg-card,.traf-mini,.traf-chart-card,.create-panel,.cp-block,.conn-hero-tile,
.modal,.proto-card,.srv-tile,.cfgdash-card,.status-big{
  backdrop-filter:blur(var(--glass-blur)) saturate(150%);
  -webkit-backdrop-filter:blur(var(--glass-blur)) saturate(150%);
}
.card,.cfg-card,.traf-mini,.traf-chart-card,.conn-hero-tile{box-shadow:0 4px 22px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.04)}
::-webkit-scrollbar{width:5px;height:5px}
::-webkit-scrollbar-track{background:var(--bg)}
::-webkit-scrollbar-thumb{background:var(--bg3);border-radius:3px}
a{color:inherit;text-decoration:none}
.sidebar{width:var(--sidebar-w);min-height:100vh;background:linear-gradient(180deg,rgba(18,20,32,.85),rgba(10,11,20,.92));border-left:1px solid var(--card-b);display:flex;flex-direction:column;flex-shrink:0;position:fixed;right:0;top:0;bottom:0;z-index:200;transition:transform .25s cubic-bezier(.4,0,.2,1),background .3s,border-color .3s}
...
.nav-badge-indep{background:rgba(34,197,94,0.16);color:#22e58a;box-shadow:0 0 8px rgba(34,197,94,.35)}
/* ── آکاردئون «تنظیمات سیستم» ── */
.nav-acc{margin:6px 6px 2px}
.nav-acc-head{display:flex;align-items:center;gap:9px;padding:9px 8px;border-radius:9px;color:var(--t3);
  font-size:11.5px;font-weight:700;letter-spacing:.02em;cursor:pointer;transition:all .18s;user-select:none}
.nav-acc-head:hover{background:var(--accent-d);color:var(--t2)}
.nav-acc-head i:first-child{font-size:15px;width:18px;text-align:center}
.nav-acc-chevron{margin-right:auto;font-size:14px!important;width:auto!important;transition:transform .28s cubic-bezier(.4,0,.2,1)}
.nav-acc.open .nav-acc-chevron{transform:rotate(180deg)}
.nav-acc.open .nav-acc-head{color:var(--t1);background:linear-gradient(90deg,rgba(108,92,231,.14),transparent)}
.nav-acc-body{max-height:0;overflow:hidden;transition:max-height .32s cubic-bezier(.4,0,.2,1)}
.nav-acc-inner{padding:2px 0 4px;position:relative}
.nav-acc-inner:before{content:'';position:absolute;top:2px;bottom:6px;right:19px;width:1px;
  background:linear-gradient(180deg,rgba(108,92,231,.5),transparent)}
.nav-it.sub{padding-right:30px;font-size:11.5px;margin:1px 6px;position:relative}
.nav-it.sub:before{content:'';position:absolute;right:19px;top:50%;width:6px;height:1px;background:var(--card-b)}
.nav-it.sub.on{background:linear-gradient(90deg,rgba(108,92,231,.18),transparent);border-right-color:var(--accent)}
/* ── انیمیشن باز/بسته شدن منو + افکت محو حرفه‌ای ── */
.sidebar{transition:transform .32s cubic-bezier(.16,1,.3,1),box-shadow .32s}
.sidebar .nav-it,.sidebar .nav-acc,.sidebar .nav-sec{opacity:0;transform:translateX(10px);transition:opacity .32s ease,transform .32s ease}
.sidebar.open .nav-it,.sidebar.open .nav-acc,.sidebar.open .nav-sec{opacity:1;transform:translateX(0)}
.sidebar.open .nav-it:nth-child(1){transition-delay:.04s}
.sidebar.open .nav-it:nth-child(2){transition-delay:.07s}
.sidebar.open .nav-it:nth-child(3){transition-delay:.10s}
.sidebar.open .nav-it:nth-child(4){transition-delay:.13s}
.sidebar.open .nav-it:nth-child(5){transition-delay:.16s}
...
@media(max-width:500px){
  ...
  button,.btn,.tab-btn,.nav-it,.proto-card,input,select{min-height:38px}
  .btn,.cp-submit-btn{padding:11px 16px}
...
<aside class="sidebar" id="sb">
  ...
  <div class="logo">
    ...
    <div><div class="logo-name">X5G</div><div class="logo-sub">5.2 LevelUp</div></div>
  </div>
  <div class="nav-wrap">
    ...
    <div class="nav-it" data-pg="users"><i class="ti ti-users"></i> کاربران <span class="nav-badge" id="users-nb">0</span></div>
    ...
    <div class="nav-acc" id="acc-system">
      <div class="nav-acc-head" onclick="toggleNavAcc('system')">
        ...
        <i class="ti ti-chevron-down nav-acc-chevron"></i>
      </div>
      <div class="nav-acc-body" id="acc-system-body">
        <div class="nav-acc-inner">
          <div class="nav-it sub" data-pg="security"><i class="ti ti-shield-lock"></i> امنیت</div>
          <div class="nav-it sub" data-pg="logs"><i class="ti ti-history"></i> لاگ فعالیت‌ها</div>
          <div class="nav-it sub" data-pg="errors"><i class="ti ti-alert-triangle"></i> خطاها</div>
          <div class="nav-it sub" data-pg="testws"><i class="ti ti-wifi"></i> تست WebSocket</div>
          <div class="nav-it sub" data-pg="settings"><i class="ti ti-settings"></i> تنظیمات کلی</div>
          <div class="nav-it sub" data-pg="support"><i class="ti ti-headset"></i> پشتیبانی</div>
        </div>
      </div>
    </div>
  </div>
...
<section class="pg on" id="pg-overview">
  ...
  <div class="dash-footer">
    <span class="df-text">X5G 5.2 LevelUp · Railway</span>
    ...
    <div class="cp-body">
      ...
      <div class="cp-row mb16">
        <div class="cp-block" style="flex:1">
          <div class="cp-block-label"><i class="ti ti-gauge"></i> محدودیت سرعت (هر دو جهت)</div>
          <div class="form-row">
          ...
          <div class="hint" style="margin-top:6px">اگه سرعت آپلود/دانلود رو جدا پایین‌تر تنظیم کنی، این مقدار مشترک نادیده گرفته می‌شه.</div>
        </div>
      </div>
      <div class="cp-row mb16">
        <div class="cp-block">
          <div class="cp-block-label"><i class="ti ti-arrow-up"></i> محدودیت سرعت آپلود (اختصاصی)</div>
          <div class="form-row">
            <input class="cp-input-full" id="nl-upload" type="number" min="0" step="0.5" placeholder="0 = از مقدار مشترک بالا" value="0" style="flex:1">
            <select class="fs" id="nl-upload-unit" style="flex:0 0 100px">
              <option value="MBIT" selected>Mbps</option>
              <option value="KB">KB/s</option>
              <option value="MB">MB/s</option>
            </select>
        ...
        <div class="cp-block">
          <div class="cp-block-label"><i class="ti ti-arrow-down"></i> محدودیت سرعت دانلود (اختصاصی)</div>
          <div class="form-row">
            <input class="cp-input-full" id="nl-download" type="number" min="0" step="0.5" placeholder="0 = از مقدار مشترک بالا" value="0" style="flex:1">
            <select class="fs" id="nl-download-unit" style="flex:0 0 100px">
              <option value="MBIT" selected>Mbps</option>
              <option value="KB">KB/s</option>
              <option value="MB">MB/s</option>
            </select>
      ...
      <div class="cp-row mb16" style="grid-template-columns:1fr">
        <div class="cp-block">
          <div class="cp-block-label"><i class="ti ti-user"></i> تخصیص به کاربر (اختیاری)</div>
          <select class="cp-input-full fs" id="nl-owner">
            <option value="">بدون تخصیص</option>
          </select>
          <div class="hint" style="margin-top:6px">از تب «کاربران» می‌تونی کاربر جدید بسازی؛ این لیست خودکار پر می‌شه.</div>
        </div>
      </div>
      <div class="cp-footer">
...
<section class="pg" id="pg-v2">
  <div class="topbar">
    <div><div class="tb-title"><i class="ti ti-satellite"></i> V2 — سابسکریپشن هوشمند</div><div class="tb-sub">Pool پایدار بهترین IPها + خروجی sing-box با failover خودکار</div></div>
    <div class="tb-right"><button class="btn btn-o btn-sm" onclick="v2LoadPool()"><i class="ti ti-refresh"></i> رفرش Pool</button></div>
  </div>
  <div class="card" style="margin-bottom:14px">
    <p style="font-size:12px;color:var(--t2);line-height:1.9;margin:0">
      این لیست از هر اسکنی که تو تب <b>Scanner</b> بزنی، خودکار merge می‌شه (نه overwrite — با میانگین متحرک، پس یه افت موقتی همه‌چیزو خراب نمی‌کنه). دکمه‌ی پایین یه subscription واقعی <b>sing-box</b> می‌سازه که خودش،‌ بدون نیاز به کاری از سمت تو، بین این IPها با گروه <code>urltest</code> (هر ۱۰ ثانیه تست، بدون صبر طولانی) سوئیچ می‌کنه.
    </p>
  </div>
  <div class="card" style="margin-bottom:14px">
    <div style="font-size:12px;font-weight:700;color:var(--t1);margin-bottom:10px"><i class="ti ti-link"></i> ساخت Subscription</div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
      <input id="v2-uuid" placeholder="UUID کانفیگ" style="background:rgba(0,0,0,.25);border:1px solid var(--card-b);color:var(--t1);padding:9px;border-radius:8px;font-size:12px;direction:ltr">
  ...
  <div class="card">
    ...
    <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:11.5px;direction:ltr">
      <thead><tr style="color:var(--t3);text-align:left"><th style="padding:6px">IP</th><th>Score</th><th>Avg ms</th><th>Jitter</th><th>Success</th></tr></thead>
      <tbody id="v2-pool-body"></tbody>
    </table></div>
  </div>
</section>
...
<section class="pg" id="pg-users">
  <div class="topbar">
    <div><div class="tb-title"><i class="ti ti-users"></i> کاربران</div><div class="tb-sub">مدیریت کاربران و تخصیص کانفیگ‌ها — مصرف کل هر کاربر رو اینجا ببین</div></div>
    <div class="tb-right"><span class="badge bg-blue" id="users-pg-cnt">۰ کاربر</span></div>
  </div>
  <div class="create-panel">
    <div class="cp-head">
      <div class="cp-head-icon"><i class="ti ti-user-plus"></i></div>
      <div class="cp-head-text">
        <div class="cp-head-title">کاربر جدید</div>
        <div class="cp-head-sub">بعد از ساخت، از تب «کانفیگ‌ها» می‌تونی این کاربر رو به یک کانفیگ تخصیص بدی</div>
      </div>
    </div>
    <div class="cp-body">
      <div class="cp-row">
        <div class="cp-block">
          <div class="cp-block-label"><i class="ti ti-id-badge-2"></i> نام کاربر</div>
          <input class="cp-input-full" id="nu-name" placeholder="مثلاً: علی رضایی">
        </div>
        <div class="cp-block">
          <div class="cp-block-label"><i class="ti ti-note"></i> یادداشت (اختیاری)</div>
          <input class="cp-input-full" id="nu-note" placeholder="مثلاً: شماره تماس یا توضیح">
        </div>
      </div>
      <div class="cp-footer">
        <div class="cp-footer-note"><i class="ti ti-info-circle"></i> کاربر فقط برای دسته‌بندی و دیدن مصرف کلیه — رمز ورود جدایی نداره.</div>
        <button class="cp-submit-btn" onclick="createUser()"><i class="ti ti-user-plus"></i> ساخت کاربر</button>
      </div>
    </div>
  </div>
  <div class="cfg-grid" id="users-grid"></div>
  <div class="empty" id="users-empty" style="display:none"><i class="ti ti-users"></i><p>هنوز کاربری ساخته نشده</p></div>
</section>
  ...
  <div class="g2">
    <div class="srv-panel">
      ...
      <div class="srv-tiles">
        ...
        <div class="srv-tile"><div class="srv-tile-icon"><i class="ti ti-versions"></i></div><div class="srv-tile-text"><div class="srv-tile-label">نسخه</div><div class="srv-tile-val">5.2 LevelUp</div></div></div>
...
const SYSTEM_PAGES=new Set(['security','logs','errors','testws','settings','support']);
function navTo(name){
...
document.querySelectorAll('.nav-it').forEach(el=>{if(el.dataset.pg)el.addEventListener('click',()=>navTo(el.dataset.pg))});
function toggleNavAcc(id){
  const acc=document.getElementById('acc-'+id);
  acc.classList.contains('open')?closeNavAcc(id):openNavAcc(id);
}
function openNavAcc(id){
  const acc=document.getElementById('acc-'+id),body=document.getElementById('acc-'+id+'-body');
  acc.classList.add('open');
  body.style.maxHeight=body.scrollHeight+'px';
}
function closeNavAcc(id){
  const acc=document.getElementById('acc-'+id),body=document.getElementById('acc-'+id+'-body');
  acc.classList.remove('open');
  body.style.maxHeight='0px';
}
async function openWorkerDashboard(){
  try{
    const r=await fetch('/api/me');const d=await r.json();
    if(d.worker_domain){window.open('https://'+d.worker_domain+'/','_blank');}
    else{toast('دامنه‌ی Worker هنوز تنظیم نشده (WORKER_PUBLIC_DOMAIN)','err');}
  }catch(e){toast('خطا در گرفتن آدرس Worker','err')}
}

// ── X5.3: V2 (Pool پایدار + Subscription) و Scanner (Neighbor Discovery) ──
function v2RowsHTML(rows){
  if(!rows.length) return '<tr><td colspan="5" style="padding:14px;color:var(--t3);text-align:center">هنوز چیزی اسکن نشده</td></tr>';
  return rows.map(r=>{
    const c = r.score>70?'#22c55e':r.score>40?'#f59e0b':'#ef4444';
    return `<tr style="border-top:1px solid var(--card-b)"><td style="padding:6px">${r.ip}</td>
      <td style="color:${c};font-weight:700">${r.score}</td><td>${r.avg_ms??'-'}</td><td>${r.jitter_ms??'-'}</td>
      <td>${r.success_rate!==undefined?Math.round(r.success_rate*100)+'%':'-'}</td></tr>`;
  }).join('');
}
async function v2LoadPool(){
  try{
    const r=await fetch('/api/v2/pool?limit=30'); const d=await r.json();
    document.getElementById('v2-pool-body').innerHTML=v2RowsHTML(d.pool||[]);
    const nb=document.getElementById('pool-nb'); if(nb) nb.textContent=(d.pool||[]).length;
  }catch(e){toast('خطا در گرفتن Pool','err')}
}
async function v2BuildSub(){
  const uuid=document.getElementById('v2-uuid').value.trim();
  const top=document.getElementById('v2-top').value||6;
  const out=document.getElementById('v2-sub-out');
  if(!uuid){toast('UUID کانفیگ رو وارد کن','err');return;}
  try{
...
async function scRun(){
  ...
  const btn=document.getElementById('sc-btn'), prog=document.getElementById('sc-progress');
  ...
  prog.textContent = neighbors? 'در حال تست کل /24 (ممکنه چند ثانیه طول بکشه)...' : 'در حال تست IP...';
  try{
    ...
    document.getElementById('sc-body').innerHTML = (d.results||[]).map(x=>{
      const c=x.ok?(x.score>70?'#22c55e':x.score>40?'#f59e0b':'#ef4444'):'#ef4444';
      return `<tr style="border-top:1px solid var(--card-b)"><td style="padding:6px">${x.ip}</td>
    ...
    prog.textContent = `${d.scanned} آدرس تست شد — نتایج خودکار تو Pool تب V2 هم ذخیره شد.`;
    v2LoadPool();
  }catch(e){ prog.textContent=''; toast(e.message||'خطا در اسکن','err'); }
...
async function loadUsers(){
  try{
    const r=await authF('/api/users');
    const {users=[]}=await r.json();
    allUsersList=users;
    document.getElementById('users-nb').textContent=users.length;
    document.getElementById('users-pg-cnt').textContent=toFa(users.length)+' کاربر';
    const grid=document.getElementById('users-grid');
    document.getElementById('users-empty').style.display=users.length?'none':'block';
    grid.innerHTML=users.map(u=>`
      <div class="cfg-card">
        <div class="cfg-row">
          <span class="cfg-status-dot pulse"></span>
          <div class="cfg-identity">
            <div class="cfg-label"><i class="ti ti-user"></i> ${esc(u.name)}</div>
            <div class="cfg-sub-meta">
              <span>${new Date(u.created_at).toLocaleDateString('fa-IR')}</span>
              ${u.note?`<span>${esc(u.note)}</span>`:''}
            </div>
          </div>
          <div class="cfg-divider-v"></div>
          <div class="cfg-usage-col">
            <div class="utxt"><span>${toFa(u.active_links_count)}/${toFa(u.links_count)} کانفیگ فعال</span><span>${u.total_used_fmt} مصرف کل</span></div>
          </div>
          <div class="cfg-divider-v"></div>
          <div class="cfg-actions">
            <button class="btn btn-sm btn-d btn-icon" onclick="deleteUser('${u.id}')" title="حذف"><i class="ti ti-trash"></i></button>
          </div>
        </div>
      </div>`).join('');
    populateOwnerSelect();
...
function populateOwnerSelect(){
  const sel=document.getElementById('nl-owner');
  if(!sel)return;
  const cur=sel.value;
  sel.innerHTML='<option value="">بدون تخصیص</option>'+allUsersList.map(u=>`<option value="${u.id}">${esc(u.name)}</option>`).join('');
  sel.value=cur;
}
async function createUser(){
  const name=document.getElementById('nu-name').value.trim();
  const note=document.getElementById('nu-note').value.trim();
  if(!name){toast('نام کاربر را وارد کنید','err');return}
  try{
    const r=await authF('/api/users',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,note})});
    if(!r.ok)throw new Error('failed');
    document.getElementById('nu-name').value='';
    document.getElementById('nu-note').value='';
    toast('کاربر ساخته شد ✓','ok');loadUsers();
  }catch(e){toast('خطا در ساخت کاربر','err')}
}
async function deleteUser(id){
  if(!confirm('این کاربر حذف بشه؟ کانفیگ‌هاش حذف نمی‌شن، فقط تخصیصشون برداشته می‌شه.'))return;
  try{
    const r=await authF('/api/users/'+id+'?unassign_only=true',{method:'DELETE'});
    if(!r.ok)throw new Error('failed');
    toast('کاربر حذف شد','ok');loadUsers();loadLinks();
  ...
  return `<div class="cfg-card ${cardCls}">
    <div class="cfg-row">
      ...
      <div class="cfg-badges-col">
        ...
        <span class="cfg-sub-tag" title="محدودیت سرعت آپلود/دانلود"><i class="ti ti-gauge"></i> ${l.upload_limit_bytes||l.download_limit_bytes?((l.upload_limit_bytes?(l.upload_limit_bytes*8/1024/1024).toFixed(1):'∞')+'↑/'+(l.download_limit_bytes?(l.download_limit_bytes*8/1024/1024).toFixed(1):'∞')+'↓ Mbps'):(l.speed_limit_bytes?((l.speed_limit_bytes*8/1024/1024).toFixed(1)+' Mbps'):'نامحدود')}</span>
        ${l.owner_name?`<span class="cfg-sub-tag" title="کاربر تخصیص‌یافته"><i class="ti ti-user"></i> ${esc(l.owner_name)}</span>`:''}
      </div>
      <div class="cfg-divider-v"></div>
      <div class="cfg-actions">
...
async function createLink(){
  ...
  const upload_limit_value=Number(document.getElementById('nl-upload').value)||0;
  const upload_limit_unit=document.getElementById('nl-upload-unit').value;
  const download_limit_value=Number(document.getElementById('nl-download').value)||0;
  const download_limit_unit=document.getElementById('nl-download-unit').value;
  const owner_user_id=document.getElementById('nl-owner').value||null;
  try{
    const r=await authF('/api/links',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({label,limit_value:val||0,limit_unit:unit,expires_days:exp||0,note,protocol,fingerprint,alpn,port,ip_limit,speed_limit_value,speed_limit_unit,upload_limit_value,upload_limit_unit,download_limit_value,download_limit_unit,owner_user_id})});
    if(!r.ok)throw new Error('failed');
    ...
    document.getElementById('nl-upload').value='0';
    document.getElementById('nl-download').value='0';
...
function refreshAll(){fetchStats();loadLinks();loadXrayStatus();if(document.getElementById('pg-connections').classList.contains('on'))loadConns();if(document.getElementById('pg-logs').classList.contains('on'))loadActivity();if(document.getElementById('pg-users').classList.contains('on'))loadUsers();toast('رفرش شد','ok')}
...
<div class="wrap">
  ...
  <div class="footer">پشتیبانی: <a href="https://t.me/X4GHUB" target="_blank">@X4GHUB</a> · X5G 5.2 LevelUp</div>
</div>
<script>
