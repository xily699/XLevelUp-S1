...
function navTo(name){
  ...
  const loaders={links:loadLinks,connections:loadConns,errors:loadErrs,logs:loadActivity,cfgdash:loadCfgDash,users:loadUsers};
...
function refreshAll(){fetchStats();loadLinks();if(document.getElementById('pg-connections').classList.contains('on'))loadConns();if(document.getElementById('pg-logs').classList.contains('on'))loadActivity();if(document.getElementById('pg-users').classList.contains('on'))loadUsers();toast('رفرش شد','ok')}
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  fetchStats();loadUsers();loadLinks();
