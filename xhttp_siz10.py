...
def _tune_socket(writer: asyncio.StreamWriter):
    ...
    + SO_KEEPALIVE برای اینکه سوکت‌های نیمه‌قطع (نه واقعاً بسته، ولی مقصد دیگه جواب
    نمی‌ده — رایج در شبکه‌های موبایل/NAT) زودتر شناسایی و رها بشن، نه اینکه تا ابد
    منابع سرور رو نگه دارن."""
...
# ── X5G.1: این دو کلاس دیگه اینجا تعریف نمی‌شن — منطق‌شون به adaptive_flow.py
# منتقل شده تا relay_vless.py (مسیر اصلی VLESS-WS) هم بتونه از همون هوش
# adaptive استفاده کنه. این‌ها فقط wrapper نازکی هستن که همون امضای قدیمی
# _QuotaGate(uuid) / _AdaptiveFlow() رو حفظ می‌کنن تا بقیه‌ی این فایل بدون
# تغییر کار کنه (رفتار عیناً همونیه که بود).
from adaptive_flow import AdaptiveQuotaGate, AdaptiveFlowControl


class _QuotaGate(AdaptiveQuotaGate):
    def __init__(self, uuid: str):
        super().__init__(uuid, check_and_use)


class _AdaptiveFlow(AdaptiveFlowControl):
    pass
    ...
    try:
        while True:
            ...
            if not await gate.add(len(data)):
                break
            await throttle_download(uuid, len(data))
            async with XHTTP_LOCK:
                sess = xhttp_sessions.get(session_id)
            if sess:
...
async def packet_up_upload(uuid: str, session_id: str, seq: int, request: Request):
    ...
    await throttle_upload(uuid, len(body))
    ...
    try:
        async for chunk in request.stream():
            ...
            if not await gate.add(len(chunk)):
                raise HTTPException(status_code=403, detail="quota/disabled/unknown")
            await throttle_upload(uuid, len(chunk))
